  ...
  switch (action.type) {
    ...
    case ACTIONS.SET_CAUGHT_SHINY:
      ...
      if (isAlreadyInList) {
        ...
        axios.get("http://localhost:8080/collection/1").then((res) => {
          updatedCollectionShiny = res.data;
        });
      }

      // Send a POST request to update the backend with the new caught_shiny value
      // You need to implement the backend update logic here
